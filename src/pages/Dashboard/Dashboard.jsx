...
const Dashboard = () => {
  ...
  const chartData = [
    { date: "11.04", value: 0 }, { date: "12.04", value: 3 },
    { date: "13.04", value: 2 }, { date: "14.04", value: 2 },
    { date: "15.04", value: 1 }, { date: "16.04", value: 0 },
    { date: "17.04", value: 0 },
  ];

  const activities = [
    { name: "Alisher Karimov", action: "Darsga kechikish", desc: "10 daqiqa kechikdi", score: "-1", date: "15.04.2026 08:15", initials: "AK", type: "negative" },
    { name: "Madina Yusupova", action: "A'lo baholar", desc: "Barcha fanlarda a'lo natijalar", score: "+3", date: "10.04.2026 10:00", initials: "MY", type: "positive" },
  ];
  ...
  return (
    <div className="w-full max-w-400 mx-auto p-4 md:p-8 bg-[#F8FAFC] min-h-screen">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {stats.map((item, idx) => (
          <div key={idx} className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm h-33.75 flex flex-col justify-between hover:shadow-md transition-shadow">
      ...
      <div className="bg-white p-8 rounded-2xl border border-gray-100 shadow-sm w-full mb-8">
        ...
        <div className="relative flex h-80 w-full border-b border-gray-200 ml-8 pr-8">
          ...
          <div className="absolute -left-12 h-full flex flex-col justify-between text-[11px] text-gray-400 pb-1 text-right w-10 font-bold">
            <span>3</span><span>2.25</span><span>1.5</span><span>0.75</span><span className="translate-y-1">0</span>
          </div>
          {/* Bars Area */}
          <div className="flex-1 flex items-end">
            {chartData.map((bar, idx) => (
              <div key={idx} className="group relative flex-1 h-full flex flex-col items-center justify-end px-2" onMouseEnter={() => setHoveredBar(idx)} onMouseLeave={() => setHoveredBar(null)}>
                ...
                <div className="bg-[#5b52f0] w-full max-w-[40px] rounded-t-lg relative z-10 transition-all duration-300" style={{ height: `${(bar.value / 3) * 100}%` }}>
                  {hoveredBar === idx && (
                ...
                <span className="absolute -bottom-8 text-[11px] text-gray-400 font-bold uppercase tracking-tighter">{bar.date}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden mb-10">
        ...
        <div className="flex flex-col">
          {activities.map((item, index) => (
            <div key={index} className="flex items-start justify-between p-6 border-b border-gray-50 last:border-0 hover:bg-gray-50 transition-colors">
              <div className="flex gap-4">
                <div className="w-11 h-11 rounded-full bg-[#EEF2FF] flex items-center justify-center text-[#818CF8] text-[13px] font-bold shrink-0">{item.initials}</div>
                <div>
                  <h4 className="text-[15px] font-bold text-gray-800 leading-tight">{item.name}</h4>
                  <p className="text-[14px] text-gray-500 mt-1 font-medium">{item.action}</p>
                </div>
              </div>
              <div className="flex flex-col items-end gap-2">
                <span className={`px-4 py-1 rounded-full text-[13px] font-bold ${item.type === "positive" ? "bg-emerald-50 text-emerald-600" : "bg-rose-50 text-rose-600"}`}>{item.score}</span>
